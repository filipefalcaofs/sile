...
function PasswordField({ id, name, label, autoFocus = false, error }: FieldProps) {
    const [show, setShow] = useState(false);

    return (
        <div className="mt-[22px] grid gap-2.5">
            <label htmlFor={id} className={monoLabel}>{label}</label>
            <div className="relative">
                ...
                <button
                    ...
                    className="absolute top-1/2 right-2 grid size-10 -translate-y-1/2 cursor-pointer place-items-center rounded-lg text-[oklch(67%_0.025_250)] hover:text-[oklch(96%_0.008_250)] focus-visible:outline-none"
                >
                    {show ? <EyeIcon className="size-[18px]" /> : <EyeCloseIcon className="size-[18px]" />}
                </button>
            </div>
            {error && <p className="text-[12.5px] text-[oklch(72%_0.16_25)]">{error}</p>}
        </div>
    );
}

export default function GestaoResetPassword({ email, token }: Props) {
    return (
        <main className="grid min-h-screen place-items-center bg-[oklch(15%_0.034_255)] px-5 py-10 font-[-apple-system,BlinkMacSystemFont,'SF_Pro_Text','Segoe_UI',system-ui,sans-serif] text-[oklch(96%_0.008_250)] antialiased">
            <Head title="Redefinir senha — Gestão SEDUR">
            ...
            <section className="w-full max-w-[420px] rounded-2xl border border-white/8 bg-[oklch(19%_0.038_255)] p-[clamp(28px,5vw,44px)]">
                ...
                <h1 className={`${displayFont} text-[26px] leading-[1.1] font-extrabold tracking-[-0.02em]`}>Redefinir senha</h1>
                <p className="mt-2.5 text-sm text-[oklch(67%_0.025_250)]">Defina uma nova senha para voltar a acessar o console.</p>

                <Form action="/gestao/reset-password" method="post">
                    {({ errors, processing }) => (
                        <>
                            ...
                            <PasswordField id="password" name="password" label="Nova senha" autoFocus error={errors.password} />
                            <PasswordField id="password_confirmation" name="password_confirmation" label="Confirmar nova senha" error={errors.password_confirmation} />
