# Roadmap: Viabiliza — Sistema de Licenciamento Eletrônico

## Overview

O Viabiliza será construído em 15 fases que partem da fundação (identidade, acesso e trilha de auditoria transversal) e dos cadastros estruturantes (CNAEs, usuários, perfis, parâmetros), passam pelos dados territoriais e pelos dois motores de decisão — regras da LOUOS (Lei nº 9.148/2016) e classificação de risco (Decreto nº 32.636/2020) —, sempre parametrizáveis com regras como dados versionados, e então entregam a jornada completa do licenciamento: consulta prévia, solicitação formal, fluxo expresso automático e análise técnica humana com pendências e comunicação. Fecham o ciclo a auditoria/compliance sobre a trilha registrada desde a Fase 1, as integrações reais validadas contra homologação (REDESIM, SEFAZ, GIS, legado), a camada de inteligência artificial e os relatórios gerenciais.
...
- [ ] **Phase 14: Inteligência Artificial** — OCR, resumos, sugestão de parecer e assistentes (EP14) — EM ANDAMENTO. **Onda 0 (Fundação) CONCLUÍDA em 2026-06-16 (guardião-entrega APROVADO):** configuração administrável de servidores de e-mail (HU-014/ConfigEmail) + configuração multi-provider de IA administrável (`AiConfiguration`: CRUD sob `manter-config-ia`, credencial criptografada e mascarada/`$hidden`+DTO, teste de conexão HTTP real com anti-SSRF [https + allowlist de hosts + sem redirects + timeout com teto] e resposta sanitizada, 7 toggles `features.ia_*` default OFF). Inclui o **SDK `laravel/ai` ^0.8.1 instalado** (advisory transitivo do `phpseclib` — pré-existente via socialite — corrigido para 3.0.55; `composer audit` limpo) e a **ponte runtime** (`AiConfigResolver`/`AiConfigServiceProvider`) que aplica a config do banco em `config('ai.*')` com as chaves reais do SDK (`ai.default`, `ai.providers.{n}.driver|key|url|models`) — a tela de configuração controla o SDK de verdade, com `api_key` cacheada como ciphertext e degradação honesta sem config. Verificação fresca: suíte global 1493/1493 SQLite + 29/29 postgis; tsc/build verdes; 31 permissões / 97 parâmetros. **Onda 1 (Documentos HU-112-115) ENTREGUE em 2026-06-17 (guardião APROVADO):** provedor real cadastrado (SimplificaIA/openai/gpt-5.4-mini) + **VALIDAÇÃO REAL** da integração (chamada de geração de verdade pela ponte, tokens 37/14 — gate de homologação da fundação fechado); OCR/ilegibilidade (`LeituraDocumentoAgent`, visão), classificação, inconsistências — como **sugestão revisável** (status nunca decisão), toggle OFF ⇒ no-op auditado (não chama provedor), + card read-only "Alertas de IA" na ficha (não toca `AnalysisRecord`/RN-003). `--group ia` 13/13; suíte global 1535/1535; `composer audit` limpo. **Onda 2 (Síntese HU-116-119) ENTREGUE em 2026-06-17 (guardião APROVADO):** resumo da solicitação (portal) + resumo do processo (ficha) + sugerir minuta de parecer (NÃO-DECISÃO provada — nunca grava decisão, não despacha sem motor, recusa ficha finalizada) + explicar ao cidadão (portal, acessível eMAG). `--group ia` 32/32; suíte global 1554/1554. **Onda 3 (Assistentes HU-120-121) segue** — gates: `SimilaritySearch`/pgvector incompleto no SDK v0.8.1 (implementação própria de similaridade + extensão pgvector no PostgreSQL de produção), datasets SEDUR, smoke real. Pendências transversais das Ondas 1-2: smoke real de visão/texto contra provedor de homologação + calibração com datasets SEDUR/DPA-LGPD.
- [x] **Phase 15: Relatórios e Indicadores** — Dashboard executivo e relatórios exportáveis (EP15) — CONCLUÍDA em 2026-06-16 (15/15 planos, 7 waves; guardião-entrega APROVADO). Indicadores de viabilidade/tempo/produtividade/quedas + dashboard executivo (KPIs reais, sem delta inventado) + contrato único de export CSV/XLSX/PDF (HU-131) com retenção parametrizada e pruning agendado + retrofit das listagens (Fases 1-2) e dos CSVs históricos (auditoria/processos) no contrato único + captura de quedas do expresso (HU-145) + cadastro de feriados (HU-137). Tudo sobre dado real com degradação honesta (zona→bairro, "meta não definida", feriados não oficiais, produtividade anônima por default). Verificação fresca: 126/126 (Relatorios) + 29/29 (postgis); tsc/build verdes; 90 parâmetros / 29 permissões. Bloqueios honestos remanescentes: zona oficial GIS, lista de feriados municipais, meta da taxa expressa, produtividade nominal/colunas SAPS/PII = pendências SEDUR/DPO; entrada na sidebar depende do commit da Fase 14 (telas acessíveis por rota/Cmd+K/dashboard); smoke navegável humano pendente.
...
**Success Criteria** (o que deve ser VERDADE):
  ...
  4. Analista **visualiza** DAM de viabilidade quando emitido pela SEFAZ (HU-071); status de pagamento sincronizado quando aplicável (HU-072) — **sem geração de DAM** no Viabiliza para fluxo Regin. **[BLOQUEADO → Fase 13]** — escopo SEFAZ não confirmado; nenhum contrato agora (registrado, não simulado; 08-16).
...
| Base GIS oficial — Oracle Spatial via **GeoServer SEDUR** (WFS/WMS; ex-"SIGIS/CA 2000/S69") | HU-107, HU-031, EP04 | 4, 13 | **Fonte identificada (2026-06-15)**; acesso externo bloqueado (firewall PMS) — aguardando liberação de rede p/ IP do Viabiliza + confirmação de camadas/credenciais (NTI/SEDUR) |
...
| 14. Inteligência Artificial | Ondas 0-2 ✓ | Em andamento (Ondas 0 fundação + 1 Documentos + 2 Síntese ENTREGUES — guardião APROVADO + validação real; Onda 3 Assistentes/pgvector segue: gate pgvector + dataset SEDUR/smoke) | 2026-06-17 (parcial) |
| 15. Relatórios e Indicadores | 15/15 | Complete (guardião APROVADO) | 2026-06-16 |
