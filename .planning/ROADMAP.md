...
- [ ] **Phase 14: Inteligência Artificial** — OCR, resumos, sugestão de parecer e assistentes (EP14) — EM ANDAMENTO. **Onda 0 (Fundação) CONCLUÍDA em 2026-06-16 (guardião-entrega APROVADO):** configuração administrável de servidores de e-mail (HU-014/ConfigEmail) + configuração multi-provider de IA administrável (`AiConfiguration`: CRUD sob `manter-config-ia`, credencial criptografada e mascarada/`$hidden`+DTO, teste de conexão HTTP real com anti-SSRF [https + allowlist de hosts + sem redirects + timeout com teto] e resposta sanitizada, 7 toggles `features.ia_*` default OFF). Verificação fresca: suíte global 1484/1484 SQLite + 29/29 postgis; tsc/build verdes; 31 permissões / 97 parâmetros. **Ondas 1-3 (Documentos/Síntese/Assistentes, HU-112 a 121) e a Task 6 do 14-01 (SDK `laravel/ai` + ponte runtime) BLOQUEADAS — 3 gates de terceiros (anti-fachada, registrar nunca simular):** (1) **aprovação humana da dependência** `laravel/ai` (v0.8.1 — API 0.x) que puxa `aws-sdk-php` com 1 advisory de segurança (+ parecer do CVE); (2) **credencial de provedor de IA real** para validar contra homologação com evidência de chamada real; (3) **datasets rotulados pela SEDUR** (analista sênior + DPO) p/ golden cases/calibração + DPA e base legal LGPD do tratamento por provedor externo.
...
| 14. Inteligência Artificial | Onda 0 ✓ | Em andamento (Onda 0/fundação CONCLUÍDA — guardião APROVADO; Ondas 1-3 + SDK bloqueados: aprovação de dependência + credencial real + dataset SEDUR/DPO) | 2026-06-16 (parcial) |
