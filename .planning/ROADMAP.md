...
- [ ] **Phase 14: Inteligência Artificial** — OCR, resumos, sugestão de parecer e assistentes (EP14) — EM ANDAMENTO (fundação iniciada). Entregue até 2026-06-16: configuração administrável de servidores de e-mail (HU-014/ConfigEmail — CRUD `EmailServer` sob `manter-config-email`, senha criptografada e nunca reexibida, teste de conexão real, ponte `MailConfigServiceProvider`; commit `9dcf020`) e a fundação de IA multi-provider (Task 1 do 14-01: model `AiConfiguration` com `api_key` criptografada + padrão por capacidade em transação; commit `fe296a1`). Suíte global 1465/1465 + 29/29 postgis verde (30 permissões). PENDENTE: Tasks 2-6 do 14-01 (CRUD/UI de IA, permissão `manter-config-ia`, teste de conexão real, integração `laravel/ai` + ponte runtime) e as Ondas 1-3 do AI-SPEC (OCR/classificação/inconsistências/resumo/parecer/assistente). Sem bloqueio externo declarado (credenciais + LGPD confirmados no 14-01).
...
| 14. Inteligência Artificial | Fundação | Em andamento (config-email HU-014 + AiConfiguration; Tasks 2-6 e Ondas 1-3 pendentes) | - |
