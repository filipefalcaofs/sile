...
## Objetivo
Converter endereço em coordenada.

## História de Usuário
**Como** sistema,  
**quero** geocodificar endereço,  
**para** identificar localização do imóvel.
...
## Fluxo Principal
1. Usuário ou sistema inicia a funcionalidade **Geocodificar endereço**.
...
**Quando** executar a funcionalidade **Geocodificar endereço**,  
...
Implementação de referência no projeto SIGVISA (`sls-sms`): geocodificação via Nominatim/OpenStreetMap (`NominatimClient`, `GeocodingService`), processamento assíncrono por job, comando de geocodificação em lote, enriquecimento de endereço e consulta de CEP (`CepService`), com exibição em mapa Leaflet. Avaliar com a SEDUR se a base GIS municipal substitui ou complementa o provedor público.
