...
- **Parametrização máxima (HU-014)**: o administrador NUNCA deve depender de desenvolvedor para mudar o que pode ser parâmetro — e parametrização máxima é a parametrização do que FAZ SENTIDO parametrizar, não de tudo. Nenhum valor de negócio hardcoded (prazos, limiares, taxas, textos, e-mails, termos) — tudo em parâmetros administráveis por interface, com validação, histórico auditado e efeito sem deploy. Toda funcionalidade acoplável (integrações, fluxo expresso, IA, canais de notificação, aprovação automática) nasce com feature toggle administrável; desativação degrada de forma controlada (pendência/aviso explícito, nunca falha silenciosa). Constantes técnicas e detalhes internos de implementação NÃO viram parâmetro. Credenciais criptografadas, com botão de teste de conexão nas telas de integração. Ao implementar qualquer HU, o critério é: "existe alguém de negócio que pode querer mudar isso sem deploy?" — então é parâmetro.
