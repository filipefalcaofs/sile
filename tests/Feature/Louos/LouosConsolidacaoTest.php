...
class LouosConsolidacaoTest extends TestCase
{
    ...
    /**
     * Planilha vigente: 4712-1/00 com P11=sim e 100 m² enquadra nR1-01.
     */
    private function seedTratamento(): void
    {
     ...
     */
    private function inputMinimercado(?TerritoryResult $territory = null, array $extra = []): EnquadramentoInput
    {
        return new EnquadramentoInput(
            ...
            territory: $territory,
            respostas: [11 => true],
            vagasDeclaradas: $extra['vagasDeclaradas'] ?? [],
        );
    }
     ...
     */
    private function territorio(array $zona, ?array $restricoes = null, ?array $via = null): TerritoryResult
    {
     ...
     */
    private function viaIdentificada(string $classe): array
    {
        return [
            'status' => 'identificado',
            'nome' => 'Via de teste',
            'propriedades' => ['CLASSE_VIA_LOUOS' => $classe],
            'motivo' => null,
            'versao_camada' => 'vias-louos-2026',
            'distancia_m' => 0,
        ];
    }

    /**
     * @param  list<string>  $condicoes
     */
    private function seedQuadro11a(string $classeVia, string $grupoUso, array $condicoes): void
    {
    ...
    {
        $this->seedTratamento();
        $this->seedQuadro10('ZPAM', 'nR1', Quadro10Permissao::Proibido);

        $result = $this->service()->enquadrar($this->inputMinimercado(
        ...
        $this->assertStringNotContainsString('Quadro 7', (string) $result->consolidado['motivo']);
    }
    ...
    public function test_fundamentacao_cita_louos_e_quadros_aplicados(): void
    {
        ...
        $this->assertStringContainsString('nR1-01', $fundamentacao);
        $this->assertStringContainsString('Quadro 10', $fundamentacao);
        $this->assertStringNotContainsString('Quadro 7', $fundamentacao);
    ...
    public function test_resultado_e_auditado_com_fundamentacao_e_versao(): void
    {
        ...
        $this->service()->enquadrar($this->inputMinimercado(
            $this->territorio($this->zonaIdentificada('ZR-1')),
        ));
        ...
        $this->assertSame('planilha-20-08-26', $activity->properties['versoes']['risco_tratamento']);
    ...
    public function test_quadro11a_nao_indeferre_mesmo_com_zona_permitida(): void
    {
        $this->seedTratamento();
        $this->seedQuadro10('ZR-1', 'nR1', Quadro10Permissao::Permitido);
        $this->seedQuadro11a('VL', 'nR1', ['Não']);

        $result = $this->service()->enquadrar($this->inputMinimercado(
            $this->territorio($this->zonaIdentificada('ZR-1'), null, $this->viaIdentificada('VL')),
        ));

        $this->assertSame(ResultadoViabilidade::NaoPermitido->value, $result->resultado());
        $this->assertNull($this->condicionantePorTipo($result->consolidado['condicionantes'], 'via'));
    }

    public function test_quadro11a_r_consolida_pendente(): void
    {
        $this->seedTratamento();
        $this->seedQuadro10('ZR-1', 'nR1', Quadro10Permissao::Permitido);
        $this->seedQuadro11a('VL', 'nR1', ['R']);
        ...
        $this->assertSame(ResultadoViabilidade::Pendente->value, $result->resultado());
        $this->assertStringContainsStringIgnoringCase('cnlu', (string) $result->consolidado['motivo']);
    }

    public function test_quadro11a_condicao_textual_vira_condicionante(): void
    {
        $this->seedTratamento();
        $this->seedQuadro10('ZR-1', 'nR1', Quadro10Permissao::Permitido);
        $this->seedQuadro11a('VL', 'nR1', ['Sim — recuo frontal de 5 m']);
        ...
        $this->assertSame(ResultadoViabilidade::PermitidoComCondicoes->value, $result->resultado());
        $this->assertNotNull($this->condicionantePorTipo($result->consolidado['condicionantes'], 'via'));
    }
    ...
    public function test_vagas_parametrizadas_sem_deploy(): void
    {
        ...
        $input = fn (): EnquadramentoInput => $this->inputMinimercado(
            $this->territorio($this->zonaIdentificada('ZR-1')),
            ['vagasDeclaradas' => ['vagas' => 2]],
        );
